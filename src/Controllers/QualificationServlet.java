		...
		if(save.equalsIgnoreCase("group")){
			...
			try {
				...
				while (allData.next()){
					...
					json.addProperty("qualificationid", allData.getString("qualificationid"));
					json.addProperty("qualificationdes", allData.getString("qualificationdes"));
					list.add(json);
				}
				response.setContentType("application/json");
			...
			if(result != 0){
				response.sendRedirect("/QualificationMaster.jsp");
