...
$(function () {
  ...
  $('.edit').click(function (e) {
	  ...
	  if(tablerow == 0){
		  ...
		  $.ajax({
			     type: "POST",
			     url: "EmployeeTypeServlet",
			     dataType : "json",
			     success: function(data){
			     	var out = '';
			     		$.each(data, function(key,value){
			     			if(value.EmpTypeId == rowno){
			     				$('#closebutton').css("display","none");
					     			...
					     			out = '<button style="margin:-10px; margin-left: 650px; background-color: #7F0A97;border-color: #7F0A97;" onclick="location.href=\'/HumanResource/EmployeeType.jsp\'">'
						     	    +'<i class="fa fa-times" style="color: #FFF;"></i></button>'
					     			+'<form action="AddEmployeeTypeServlet" method="post" id="editform" style="display: block;">'+'<div class="row">'+'<div class="form-group">'
					     			+'<div class="col-md-6" style="display: none;">'
					     			+'<input type = "hidden" name = "id" style="visibility: hidden;" value="'+value.EmpTypeId+'"></input>'
					     			+'<input type = "text" name = "save" value="edit" style="visibility: hidden;"/>'
					     		    +'</div>'
					     		    +'<div class="col-md-6">'+'<label>Employee Type:</label>'
					     		    +'<input type="text" name="employeeType" class="form-control" placeholder="City Name"value=\"'+value.EmpTypeDes+'\" required/>'+'</div>'
					     		    +'</div>'+'</div>'
 ...
 $(document).ready(function(){
	$.ajax({
	     ...
	     success: function(data){
	     	...
	     	var out = '';
	     		$.each(data, function(key,value){
	     			out = out + '<tr><td id = '+value.EmpTypeId+'><input type="checkbox" name="selrow" value = '+value.EmpTypeId+'></td>';
	     			out = out + '<td>'+value.EmpTypeDes+'</td></tr>';
	     		})
...
<center>
  <h2 ><a href="MainMaster.jsp" style="color: #7F0A97">Employee Type</a></h2>
  </center>
...
<div class="wrap push" style="left: 0px; transition: left 300ms ease;">
    ...
    <button id="closebutton" style="margin:-10px; margin-left: 650px; background-color: #7F0A97;border-color: #7F0A97; display: block;" onclick="location.href='/HumanResource/EmployeeType.jsp'">
    	<i class="fa fa-times" style="color: #FFF;"></i>
    </button>
    <form class="displayform" action="AddEmployeeTypeServlet" method="post" style="display: block;">
    <input type = "text" name = 'save' value = 'add' style="visibility: hidden;"/>
    <div class="row">
    <div class="form-group">
    <div class="col-md-6" style="justify-content: center;">
    <label>Employee Type:</label>
    <input type="text" name="employeeType" class="form-control" placeholder="Employee Type" required/>
    </div>
